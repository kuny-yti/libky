...
public:
    ...
    //!
    //! \brief ip_legality 是否合法ipv4,ipv6
    //! \param ip
    //! \return        true为合法,否则不合法
    //!
    static bool ip_legality(const ky_string &ip);
    //!
    //! \brief domain_legality 域名是否合法
    //! \param ip
    //! \return true为合法,否则不合法
    //!
    static bool domain_legality(const ky_string &domain);
...
public:
    ...
    //! \param is_ipv6
    //!
    ky_netaddr (ushort port = 0, bool is_ipv6 = false);
    //!
    ...
    //! \param port
    //!
    ky_netaddr (const ky_string &ip, ushort port);
    ...
    //! \param port
    //!
    ky_netaddr(uint32 ipHostByteOrder, uint16 port);
