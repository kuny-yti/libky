...
typedef enum
{
    Socket_ErrorNot = Io_ErrorUnknown,    ///< Socket 没有错误
    Socket_ErrorNotOpen = Io_ErrorNotOpen,///< Socket 创建错误
    Socket_ErrorNotAddress,               ///< Socket 地址无效错误
...
kyPackage isocket : iio
{
    ...
    //! \return       读取到的长度
    //!
    virtual size_t read(void *data, size_t maxlen, ky_netaddr *ip)=0;
    //!
    ...
    //! \return      返回发送的长度
    //!
    virtual size_t write(const void *data, size_t len, const ky_netaddr &ip)=0;
    ...
    //! \return
    //!
    virtual bool open(eNetworkSockets s = Socket_TCPv4)
    {
        return open((io_mode_t)s);
    }
    ...
    //! \return
    //!
    virtual bool open(io_mode_t mode = Socket_TCPv4) = 0;
    ...
    struct setup
    {
        ...
        virtual bool get_nonblocking() = 0;
        ...
        virtual bool get_keepalive(int *idle, int *interval, int *count) = 0;
        ...
        virtual bool get_reuse(bool *aaddr, bool *sport) = 0;
        ...
        //! \return
        //!
        virtual bool set_binddevice(char *dev)= 0;
        virtual char* get_binddevice() = 0;
