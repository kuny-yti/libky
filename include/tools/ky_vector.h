...
public:
    ky_vector();
    explicit ky_vector(size_t count);
...
public:
    ...
    ky_vector<T> &append(const T *s, size_t len);
    ky_vector<T> &append(const ky_vector<T> &a);

    ky_vector<T> &insert(size_t i, T c);
    ky_vector<T> &insert(size_t i, const T *s, size_t len);
    ky_vector<T> &insert(size_t i, const ky_vector<T> &a);

    void remove(size_t index, size_t len = 1);

    ky_vector<T> &replace(size_t index, size_t len, const T *s, size_t alen);
    ky_vector<T> &replace(size_t index, size_t len, const ky_vector<T> &s);

    inline ky_vector<T> mid(size_t pos, int len) const {return extract(pos, len);}
